...
// Attempt to override DNS servers to Google DNS to resolve Atlas SRV records
try {
  dns.setServers(['8.8.8.8', '8.8.4.4']);
  console.log('DNS servers set to Google DNS for SRV resolution.');
} catch (e) {
  console.warn('Could not override DNS servers.');
}
...
async function seed() {
  try {
    ...
    await admin.save();
    console.log('Admin user created (email: s@bit).');

    // 2. Add All 38 Listings
    const listingsWithOwner = LISTINGS_DATA.map(l => ({ 
      ...l, 
      userId: admin._id 
    }));
    ...
    console.log(`✅ Success! ${listingsWithOwner.length} listings have been migrated to the database.`);
    process.exit();
  } catch (err) {
